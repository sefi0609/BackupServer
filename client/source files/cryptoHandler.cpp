...
bool RSAkeys::loadFromFile() {
	long long fileSize = fileHandler.getFileSize(privFile);
	ifstream pfile(privFile, ifstream::binary);
	char* tmp = new char[fileSize];
	fileHandler.readBinaryFile(pfile, tmp, fileSize);
	...
	// remove salt
	int keyLength = length - SALT_SIZE;
...
string AESKey::encrypt(const char* plain, unsigned int length){
	...
	try {
		...
		cout << "Exception in encrypt(): " << e.what() << endl;
		return NULL;
	}
}