...
class CryptoHandler:
    ...
    def get_aes_key(self):
        try:
            # 16 bytes == 128 bit
            self._aes_key = get_random_bytes(16)
    ...
    def save_to_db(self, client_id, client_name, last_seen, db):
        try:
            db.save_to_clients(client_id, client_name, self._public_key, last_seen, self._aes_key)
        except Exception as e:
    ...
    def update_db(self, last_seen, client_id, db):
        try:
            db.update_aes_key(last_seen, self._aes_key, client_id)
        except Exception as e:
            print(f"Exception at RSAHandler - update_db(): {e}")
            raise e
