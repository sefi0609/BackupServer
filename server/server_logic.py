...
def get_all_packets(sock):
    ...
    # for SEND_FILE request we will handel the payload separately
    # in send_file_request()
    if request.code != request_code['SEND_FILE']:
        ...
        # (1) for this request the payload can't be decoded,
        # payload process will be implemented in the requests functions
        if request.code != request_code['SEND_RSA_KEY']:
...
def send_file_request(request, sock, last_seen):
    ...
    if aes_key:
        cipher_size = struct.unpack(f'<i', request.payload[:CONTENT_SIZE])
        cipher_size = cipher_size[0]
        ...
        # decrypt file by chunks and save it to the right path
        # support large files
        bytes_receive = 0
        while bytes_receive < cipher_size:
            remain = cipher_size - bytes_receive
            chunk = sock.receive_from_client()

            if remain < PACKET_SIZE:
                # last packet
                plaintext = aes.decrypt(chunk[:remain], True)
            else:
                plaintext = aes.decrypt(chunk, False)

            # append this chunk to the file
            write_file(client_name, file_name, plaintext)
            # last chunk < PACKET_SIZE, the while loop will stop
            bytes_receive += PACKET_SIZE
        ...
        payload = struct.pack(f'<{CLIENT_ID_LEN}si{NAME_LEN}si',
                              request.client_id, cipher_size, file_name.encode(), check_sum)
